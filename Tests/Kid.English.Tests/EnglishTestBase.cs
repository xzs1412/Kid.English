...
using Kid.English.Migrations.SeedData;
...
namespace Kid.English.Tests
{
    public abstract class EnglishTestBase : AbpIntegratedTestBase<EnglishTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected EnglishTestBase()
        {
        ...
        protected void UsingDbContext(Action<EnglishDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<EnglishDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<EnglishDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<EnglishDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<EnglishDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<EnglishDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<EnglishDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<EnglishDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<EnglishDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<EnglishDbContext, Task<T>> func)
        {
            T result;
